let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Todavía hay algo que hacer\nUn texto \nen este chat aun no esta hecho... esperar hasta que esté termine'
  else conn.hartatahta[m.chat] = true
  m.reply('_Creando..._\n*Espere aproximadamente 1 minuto*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*©♞☞  𝕊𝓐тคŇ  🐲🏆*\nMade with FFmpeg', m)
  } finally {
