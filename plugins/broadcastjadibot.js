let handler = async (m, { conn, text }) => {
  if (conn.user.jid === global.conn.user.jid) {
    ...
    conn.reply(m.chat, `_Transmisión enviada con éxito a ${users.length} número terminado bot_`, m)
  } else conn.reply(m.chat, 'Esta característica es solo para hosts de bots',  m)
}
