...
global.owner = 50373488366','50373488366','50363488366'] // Put your number here
...
if (!opts['test']) setInterval(() => {
  ...
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('Database is up to date')
    ...
    conn.logger.info('Done saving database,YouTube: Soft Devil!')
  ...
  try {
    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (xp > 99) m.reply('a') // Hehehe
        else m.exp += xp
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < m.limit * 1 && plugin.limit) {
          this.reply(m.chat, `Su límite se agota, por favor compre a través de *${usedPrefix}buy*`, m)
        ...
        try {
          ...
          if (m.limit) m.reply(+ m.limit + ' Límite utilizado, se removera de tus limites')
        }
  			break
  		}
  	}
...
conn.welcome = 'Hola Pimpoll@, @user!\nBienvenido al grupo @subject\n\nSi has ayudado a seguirnos..\nPon *Instagram:* @netomembreno\nDan Youtube: *Soft Devil y MR Pato*'
conn.bye = 'Adios aborto @user!'
conn.onAdd = async function ({ m, participants }) {
  ...
  for (let user of participants) {
    ...
    try {
      ...
      let text = (chat.sWelcome || this.welcome || conn.welcome || 'Holi, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', this.getName(m.key.remoteJid))
...
conn.onLeave = async function  ({ m, participants }) {
  ...
  for (let user of participants) {
    ...
    try {
      ...
      let text = (chat.sBye || this.bye || conn.bye || 'Bye aborto, @user!').replace('@user', '@' + user.split('@')[0])
      this.sendFile(m.key.remoteJid, pp, 'pp.jpg', text, m, false, {
        contextInfo: {
          mentionedJid: [user]
        }
...
conn.onDelete = async function (m) {
  ...
  Detectado @${m.participant.split`@`[0]} ha eliminado el mensaje
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Este perintag sólo puede ser utilizado por _*OWWNER!*_',
    owner: 'Este comando sólo puede ser utilizado por _*Owner Bot*_!',
    mods: 'Este comando sólo puede ser utilizado por _*Moderator*_ !',
    ...
    admin: 'Este comando es solo para administradores de grupo!',
    botAdmin: 'Haga el bot como *Admin* para utilizar este comando!'
  }[type]
